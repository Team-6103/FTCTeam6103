...
task drive()
{
  ...
  while (true)
  {
	  ...
	  if (true) {
	    ...
	    motor1Speed = joystick.joy1_y2 / JOYSTICK_Y1_MAX * topSpeed; // Map the leftMotorSpeed variable to joystick 1_y1
	    if (abs(motor1Speed) < JOYSTICK_DEAD_ZONE) motor1Speed = 0; // Make sure that the joystick isn't inside dead zone

	    motor2Speed = joystick.joy1_y1 / JOYSTICK_Y1_MAX * topSpeed; // Map the rightMotorSpeed variable to joystick 1_y2
	    if (abs(motor2Speed) < JOYSTICK_DEAD_ZONE) motor2Speed = 0; // Make sure that the joystick isn't inside dead zone

	    motor3Speed = joystick.joy2_y2  / JOYSTICK_Y1_MAX * topSpeed; // Map the armMotorSpeed variable to joystick 2_y2
	    if (abs(motor3Speed) < JOYSTICK_DEAD_ZONE) motor3Speed = 0; // Make sure that the joystick isn't inside dead zone

	    motor4Speed = joystick.joy2_y1  / JOYSTICK_Y1_MAX * topSpeed; // Map the teleMotorSpeed variable to joystick 2_y1
	    if (abs(motor4Speed) < JOYSTICK_DEAD_ZONE) motor4Speed = 0; // Make sure that the joystick isn't inside dead zone

	    motor[motor1] = motor1Speed; // Set the motor armMotor speed as armMotorSpeed
	    motor[motor2] = motor2Speed; // Set the motor leftMotor speed as leftMotorSpeed
	    motor[motor3] = motor4Speed; // Set the motor rightMotor speed as rightMotorSpeed
