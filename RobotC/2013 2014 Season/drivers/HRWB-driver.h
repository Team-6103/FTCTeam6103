...
/*
 * $Id: HRWB-driver.h 98 2012-08-04 09:02:53Z xander $
 */
 ...
 *
 * HRWB-driver.h provides an API for the HumaRobotics WiFiBlock Sensor.  This program
...
bool HRWBwriteRegMasked(tSensors link, int reg, ubyte data, ubyte mask)
{
  ...
  return writeI2C(link, HRWB_I2CRequest, 0);
}
...
bool HRWBreadReg(tSensors link, int reg, int size)
{
  ...
  if (!writeI2C(link, HRWB_I2CRequest, size))
    return false;

  return readI2C(link, HRWB_I2CReply, size);
}
