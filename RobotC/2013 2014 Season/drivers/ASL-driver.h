...
/*
 * $Id: ASL-driver.h 98 2012-08-04 09:02:53Z xander $
 */
...
int ASLreadThresholdAngle(tSensors link, int threshold, bool reversed=false);
...
int ASLreadAngle(tSensors link, byte reg, bool reversed) {
  ...
  if (!writeI2C(link, ASL_I2CRequest, 1))
    return -1;

  if (!readI2C(link, ASL_I2CReply, 1))
    return -1;
...
int ASLreadThresholdAngle(tSensors link, int threshold, bool reversed)
{
  int angle = 0;
