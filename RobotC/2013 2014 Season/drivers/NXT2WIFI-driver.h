...
/*
 * $Id: NXT2WIFI-driver.h 98 2012-08-04 09:02:53Z xander $
 */

#ifndef __N2W_H__
#define __N2W_H__
/** \file NXT2WIFI-driver.h
 * \brief Dani's WiFi Sensor
 *
 * NXT2WIFI-driver.h provides an API for Dani's WiFi Sensor.\n
 *
 * Changelog:
 * - 0.1: Initial release
 *
 ...
 * \date 12 June 2012
 * \version 0.1
 * \example NXT2WIFI-test1.c
 * \example NXT2WIFI-test2.c
 * \example NXT2WIFI-test3.c
 * \example NXT2WIFI-test4.c
 */
...
bool N2WRS485write(tMassiveArray &buf, ubyte len)
{
  ...
  {
    nxtReadRawHS((ubyte)N2Wrxbuffer, nxtGetAvailHSBytes());
...
bool N2WRS485read(tMassiveArray &buf, ubyte &len, int timeout = 100) {
  ...
  TMRsetup(rxTimer, timeout);
	...
	res = nxtReadRawHS((ubyte)buf, bytesAvailable);
...
bool RS485readLargeResponse(tMassiveArray &buf, int &len, int timeout = 100)
{
  ...
  TMRsetup(rxTimer, timeout);
	...
	while ((bytesleft > 0) && !TMRisExpired(rxTimer))
	{
		...
		nxtReadRawHS((ubyte)tmpBuff, bytesToRead);
 ...
 * @return the new 'tail' position of buf at which to append.
 */
int N2WappendToBuff(tBigByteArray &buf, const short index, string pData)
{
  if (index == 0) memset(buf, 0, sizeof(buf));
...
bool N2WSecurity(int mode, const ubyte &keypass, int keylen, int keyind) {
  int index = 0;
  index = N2WappendToBuff(N2Wtxbuffer, index, "$WFS?");
 ...
 * @return true if no error occured, false if it did
 */
bool N2WSecurityWPA2Passphrase(string passphrase) {
  tByteArray tmpArray;
 ...
 * @return true if no error occured, false if it did
 */
bool _N2WsetPar(string type, string param) {
  int index = 0;
	N2WscratchString = "$WFE?";
	index = N2WappendToBuff(N2Wtxbuffer, index, N2WscratchString);
	index = N2WappendToBuff(N2Wtxbuffer, index, type);
	index = N2WappendToBuff(N2Wtxbuffer, index, "=");
	index = N2WappendToBuff(N2Wtxbuffer, index, param);
	index = N2WappendToBuff(N2Wtxbuffer, index, "\n");
...
bool N2WreadWS(ubyte &btnID, ubyte &state)
{
  ...
  if (avail >= 5)
  {
    nxtReadRawHS((ubyte)N2Wrxbuffer, avail);
