...
/*
 * $Id: MSPPS-driver.h 98 2012-08-04 09:02:53Z xander $
 */
 ...
 *
 * MSPPS-driver.h provides an API for the Mindsensors PPS-v3 Pressure Sensor driver
 *
 * Changelog:
 * - 0.1: Initial release
 *
...
long MSPPSreadPressure(tSensors link, ubyte reg)
{
  ...
  if (!writeI2C(link, MSPPS_I2CRequest, 2))
    return 0;

  if (!readI2C(link, MSPPS_I2CReply, 2))
    return 0;
...
bool MSPPSsetRefPressure(tSensors link, int refpressure)
{
  ...
  return writeI2C(link, MSPPS_I2CRequest, 0);
}
