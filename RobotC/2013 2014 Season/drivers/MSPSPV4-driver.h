...
/*
 * $Id: MSPSPV4-driver.h 98 2012-08-04 09:02:53Z xander $
 */

#ifndef __PSPV4_H__
#define __PSPV4_H__
/** \file MSPSPV4-driver.h
 * \brief Playstation 2 Controller Sensor
 *
 * MSPSPV4-driver.h provides an API for the Mindsensors PS2 controller sensor
 ...
 * \date 30 July 2012
 * \version 0.1
 * \example MSPSPV4-test1.c
 * \example MSPSPV4-test2.c
 */
...
bool _PSPV4init(tSensors link, ubyte address = PSPV4_I2C_ADDR)
{
  ...
  if (!writeI2C(link, PSPV4_I2CRequest, 0))
  {
    return false;
  }
...
ubyte _PSPV4readByte (tSensors link, ubyte reg, ubyte address = PSPV4_I2C_ADDR)
{
  ...
  if (!writeI2C(link, PSPV4_I2CRequest, 1)) {
    return 0;
  }

  if(!readI2C(link, PSPV4_I2CReply, 1)) {
    return 0;
  }
...
bool PSPV4readButtons(tSensors link, tPSP &controllerState, ubyte address = PSPV4_I2C_ADDR)
{
  ...
  if (!writeI2C(link, PSPV4_I2CRequest, 6)) {
    return false;
  }

  if(!readI2C(link, PSPV4_I2CReply, 6)) {
    return false;
  }
...
bool PSPV4readButtonPressure(tSensors link, tPSP &controllerState,  ubyte address = PSPV4_I2C_ADDR)
{
  ...
  if (!writeI2C(link, PSPV4_I2CRequest, 12)) {
    return false;
  }

  if(!readI2C(link, PSPV4_I2CReply, 12)) {
    return false;
  }
...
long PSPV4readRawRefTXValue(tSensors link, ubyte address = PSPV4_I2C_ADDR)
{
  ...
  if (!writeI2C(link, PSPV4_I2CRequest, 4)) {
    return 0;
  }

  if(!readI2C(link, PSPV4_I2CReply, 4)) {
    return 0;
  }
